...
task main()
{
	...
	while(1) {
		...
		//send the values via bluetooth if they are different from the previous sent values
		if(prevSonar != sonarValue || prevFear != fearSensorValue) {
			prevSonar = sonarValue;
			prevFear = fearSensorValue;
			sendMessageWithParm(sonarValue, fearSensorValue, ARBITRARY_NUMBER_BECAUSE_I_DONT_KNOW_WHAT_ELSE_TO_SEND);
		}

		//receive motor controls via bluetooth
		if(messageParm[0] != 0)
		{
