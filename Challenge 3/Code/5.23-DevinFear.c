...
#pragma config(Sensor, S2,     lightSensorLeft, sensorLightActive)
#pragma config(Sensor, S3,     lightSensorRight, sensorLightActive)
#pragma config(Sensor, S4,     lightSensorEscape, sensorLightActive)
...
//one minute after each flash decrements
//timer max = 32000 (32 seconds)
//
	if(time1[T1]<3000 && fearValue>0){
		fearValue= fearValue-1;
	}
	//after one minute of no flash, increment
	if(time1[T1]>1000 && fearVal<4){
		fearValue++;
	}
