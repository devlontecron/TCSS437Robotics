...
task Approach()
{
	...
	//TODO is this threshold correct, bro?

	while(sonarDistance >= SONAR_ADJUST && sonarDistance < SONAR_THRESH) {
			motor[motorA] = 1.0 * sonarDistance;
...
task GradFollow(){
	...
	while(!canFeed){
		...
		if(leftColor && rightColor){
			...
			clearTimer(T2);
			while(leftColor && rightColor && time1[T2]<2000){
				//waiting for timer to increment to ensure correct direction
			}

			if(time1[T2]>=2000){
				canFeed = true;
			}else if(time1[T2]<=2000 && wrongWayCounter<=3){
				wrongWayCounter++;
			}else if(time1[T2]<=2000 && wrongWayCounter>=3){
...
task Feed(){
	...
	while(energyState != ENERGY_FULL && (leftColor || rightColor) ){
		...
		if(wrongWayCounter >= 2 && time1[T2] <1000) {
			wrongWayCounter = 0;
			motor[motorB] = DEFAULT_SPEED;
			motor[motorA] = -DEFAULT_SPEED;
			sleep(2000);
